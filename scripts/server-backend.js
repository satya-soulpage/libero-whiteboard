...
function startBackendServer(port) {
    ...
    io.on("connection", function (socket) {
        ...
        socket.on("addNewUser", function (content) {
            // if (allUsers.indexOf(content) == -1) {
            ...
            allUsers.forEach(function (row) {
                if (row.user == content.user && row.roomid == content.roomid) {
                    ifExisted = false;
                }
            });
            if (ifExisted) {
                var rgbColor = "rgb(" + colorGen() + "," + colorGen() + "," + colorGen() + ")";
                allUsers.push({ user: content.user, color: rgbColor, roomid: content.roomid });
            }
            socket.compress(false).broadcast.to(whiteboardId).emit("addNewUser", allUsers);
            socket.emit("addNewUser", allUsers);
            console.log(allUsers);
        });
        socket.on("changeColor", function (content) {
            socket.compress(false).broadcast.to(whiteboardId).emit("changeColor", content);
            socket.emit("changeColor", content);
        });
