...
// $("#connectedUsersCount")[0].innerText = myUsername;
...
function main() {
    ...
    signaling_socket.on("connect", function () {
        ...
        signaling_socket.on("addNewUser", function (content) { 
            
            var users='';
            $("#collapsibleNavId ul").empty();
            content.forEach(function(row){ 
                if(whiteboardId == row.roomid){
                $("#collapsibleNavId ul").append(`<li class="list-inline-item"><button class="btn rounded-circle custom-btn ml-n4" title=${row.user} style="background-color: ${row.color};"> ${row.user.split("")[0]}</button> </li>`);
                // users += `<button class='numberCircle' title=${row}>${row.split("")[0]}</button>`
                }
            })
           // $("#connectedUsersCount")[0].innerHTML = users;

        });
        signaling_socket.on("changeColor", function (content) { 
           
            $("#whiteboardCanvas").css('background-color', content);
        });
...
function showBasicAlert(html, newOptions) {
    ...
    var alertHtml = $(
        '<div class="basicalert" style="position:absolute; left:0px; width:100%; top:320px; font-family: monospace;">' +
            '<div style="width: 30%; margin: auto; background: #aaaaaa; border-radius: 5px; font-size: 1.2em; border: 1px solid gray;">' +
            '<div style="border-bottom: 1px solid #676767; background: #a3afac; padding-left: 5px; font-size: 0.8em;">' +
            options["header"] +
            ...
            '<div style="height: 50px; padding: 10px;"><button class="modalBtn okbtn" style="float: right;">' +
            options["okBtnText"] +
            "</button></div>" +
            "</div>" +
            "</div>"
...
function initWhiteboard() {
    $(document).ready(function () {
        ...
        signaling_socket.emit("addNewUser",  {"user":myUsername,"roomid":whiteboardId});
        new Picker({
            parent: $("#whiteboardColorpicker")[0],
            color: "#000000",
            onChange: function (color) { 
                
                if ($("#background-color").hasClass("active")) {
                $("#whiteboardCanvas").css('background-color', color.rgbaString);
                signaling_socket.emit("changeColor",  color.rgbaString);
